...
#
# This Script is used to list all Policy information from pxGrid using API.
#
...
# Function to Get the list of all polocies

function Get-Policies{
	try{
		$PolicyUrl = "https://" + $global:PlainpxGridServerIP + ":" + $global:PlainpxGridPort + "/ers/config/ancpolicy"
		$Policies = Invoke-RestMethod -Uri $PolicyUrl -Method Get -Headers $Header -Credential $global:pxGridCredential
		write-host "Total Policies available: "$Policies.SearchResult.total
		foreach ($resource in ($Policies.SearchResult.resources)){
			$keys = $resource | gm -type NoteProperty
			$OutputObject = [pscustomobject]@{}
			foreach ($key in $keys){
				if ($key.Name -eq "link"){
					$OutputObject | add-member -MemberType NoteProperty -Name ($key.Name).ToUpper() -Value $resource.($Key.Name).href
				}
				else{
					$OutputObject | add-member -MemberType NoteProperty -Name ($key.Name).ToUpper() -Value $resource.($Key.Name)
				}
			}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUek/zLJfDZ+iTrK/1m2lzAk0d
# LjagghebMIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# AQsxDjAMBgorBgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBQJAvI5lfhhpxkl6yTv
# 1C4fkCTttzANBgkqhkiG9w0BAQEFAASCAQCgB8wy3B8oX7mxHRZzMpAd6Gn57tMN
# EFpY+gSZDfIRtzZ+Afe5LQPhZQlWvkMEECfQUCK9XpZSv895MGU+Q8eqg5VkEdiI
# /o3b6Drl0CsKsqFR0UJvWyAbCNj6JfMJ4SeE3pM//wgNqPYT6Aeuk8zS1ZfZ83/U
# yYgdG3WhekmfT0VfYzBWhn7GsCVPI5om5V4jLV20ZS0ilUeLXyabxLaYRaQe2Oni
# DomyBoQanggGheOICpTfWomYFWmBVIxFPMKAeBGHYYt+lwMspB6riAgsEjgYOMIp
# yb6idYz1FfX+wh1e8mwD6EadxOo4Oix39LNjVthQKNTyaKubUVpZVUmboYICCzCC
...
# MQ8XDTE5MDMyODExMTU1MlowIwYJKoZIhvcNAQkEMRYEFHWSnrt3dcmqCgDQ2yZO
# BayJTEOvMA0GCSqGSIb3DQEBAQUABIIBADfnviQJoLrG+SYv2Ktp3yPwAQ40gVwl
# 70cBVY0HPiCW54MgH+Ubp5HOH/nFN+Qr48VMygJ1ewsW4WVTHGnDkpRdrIrOQavl
# 6WVgIOeTfbavDSCEVVBDY1EFvxU+iQ0A9Je5/TCefSI2nPPVndvBLNnstDwdmr6S
# 0tZgSwYnAEzHunghj9ZHBC27BtJ51q2tinO1ZNJmaHED9tv3owFd0ewOjxyf9FRb
# NXPQg19DNjYrP0cck23OEhlyVNwZnsd9lggXBZq0bx1ENi0TBRWf2ToTIhUutcGq
# 9yiSypVwJXhhUtzBF2iFUKrVg+hEy5xhLdhgMIlaHm1dO2+UHYjLOzM=
