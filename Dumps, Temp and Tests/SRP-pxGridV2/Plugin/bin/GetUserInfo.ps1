...
# This script is used to get information for a user
#
...
#Function to run response play
function Get-UsersInfo{
    $InternalUsersUrl = "https://" + $global:PlainpxGridServerIP + ":" + $global:PlainpxGridPort + "/ers/config/internaluser"
    $AdminUsersUrl = "https://" + $global:PlainpxGridServerIP + ":" + $global:PlainpxGridPort + "/ers/config/adminuser"
    try{
         $InternalUsers = Invoke-RestMethod -Uri $InternalUsersUrl -Method Get -Headers $Header -Credential $global:pxGridCredential
         $AdminUsers = Invoke-RestMethod -Uri $AdminUsersUrl -Method Get -Headers $Header -Credential $global:pxGridCredential
         $global:InternalUsersResource = $InternalUsers.SearchResult.resources
         $global:AdminUsersResource = $AdminUsers.SearchResult.resources
    }
    catch{
         $_.exception.Message 
     }   
...
function Find-User{
    $flag=0
    foreach($User in $global:InternalUsersResource){
        if($User.name -eq $UserName ){
            $UserUrl = $User.link.href
            $UserInfo = Invoke-RestMethod -Uri $UserUrl -Method Get -Headers $Header -Credential $global:pxGridCredential
			$OutputObject = [pscustomobject]@{}
			$Keys  = $UserInfo.InternalUser| gm -type NoteProperty
			foreach ($Key in $Keys){
				if (($UserInfo.InternalUser.($key.Name) -eq "") -or ($UserInfo.InternalUser.($Key.Name) -eq $null)){
					continue
				}
				else{
					if ($Key.Name -eq "link"){
						$OutputObject | add-member -MemberType NoteProperty -Name ($key.Name).ToUpper() -Value $UserInfo.InternalUser.($Key.Name).href
					}
					else{
						$OutputObject | add-member -MemberType NoteProperty -Name ($key.Name).ToUpper() -Value $UserInfo.InternalUser.($Key.Name)
						}
				}
			}
    ...
    foreach($User in $global:AdminUsersResource){
        if($User.name -eq $UserName ){
            $UserUrl = $User.link.href
            $UserInfo = Invoke-RestMethod -Uri $UserUrl -Method Get -Headers $Header -Credential $global:pxGridCredential
			$OutputObject = [pscustomobject]@{}
			$Keys  = $UserInfo.AdminUser| gm -type NoteProperty
			foreach ($Key in $Keys){
				if (($UserInfo.AdminUser.($key.Name) -eq "") -or ($UserInfo.AdminUser.($Key.Name) -eq $null)){
					continue
				}
				else{
					if ($Key.Name -eq "link"){
						$OutputObject | add-member -MemberType NoteProperty -Name ($key.Name).ToUpper() -Value $UserInfo.AdminUser.($Key.Name).href
					}
					else{
						$OutputObject | add-member -MemberType NoteProperty -Name ($key.Name).ToUpper() -Value $UserInfo.AdminUser.($Key.Name)
						}
				}
			}
			$OutputObject | format-list
    ...
    if($flag -eq 0){
        Write-Host "No user found with entered name"
    }
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUwbskIFMP/yb7dFHbS4qcBAQN
# fiegghebMIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# AQsxDjAMBgorBgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBRaKYva/wKZefHe7ciP
# pRGxPfoC7jANBgkqhkiG9w0BAQEFAASCAQBtFZ3Rti1MEtb5CQT22dUG+FVrBbD8
# pJFAnqZ1/HJB9Xr4jFKefl3tIHn5h3Hk19zTNNWg8xc3HLVr9/8dTKVobXMyJkYD
# P+ZUB7meZdtFw4RHII2/m4P3gDxLnKrYA9+w1LuBgeiCO+CV/57UsIFFvgfMFaSl
# +my0DAZDlvcF9mFICG+oezFP2ozs/+pd028wmf9hyOh60LhfzRKIVXmB0OIiq8H9
# W5K4Yj2/FjPAlQvCuabA3gk9v1k3g82IMNcgsuw4UlmlJkTBW8A4eqjAV5XElkjX
# y3qksRDCWb1STvIFZD9SaIFiDQ9p34bTLZBHpJDPiEnIor0ScuE/Vy/JoYICCzCC
...
# MQ8XDTE5MDMyODExMjI0OFowIwYJKoZIhvcNAQkEMRYEFNU8M/F8qo3ZZwF8e3h8
# BQagd/YTMA0GCSqGSIb3DQEBAQUABIIBAGQykccKz9ZtQoKBR8DBaKoIYzgNxZjC
# kf3X9CRZMHNWjNKVGj8S+zvw66d84yP1RBklkW9BBu0ZZxyegGcBroHEGmLD8r1y
# icxEEo33AyU+dq6VVEsaK0PHmdv5g3vvKsaPKCV9Ui0mPEpjs5LJ/rbdIMdK6MzF
# ep0/fLK6eLJ6k0lSfAz5WRc4HwaXwNoy8GTTBNyL+FYzLnGkPcRGFwaLFTsshuCl
# 1nXsi7q0LcMTqEwqKk3ImKXSSYdB8Fz8A1S89K5dyYFxQ+xcFGvSni1bs76ePQzy
# 1o1UmiCg7I0bV1spgM7OuF4SlU3iXUVf4gtCH1lt7cxdxrgj2azfRjc=
